...
def compute_cond_p(ls_filtered: DataFrame, hits_per_b: DataFrame) -> DataFrame:
    hits_per_bh = ls_filtered.groupBy("app", "host").agg(F.sum("hits").alias("bh_hits"))
    return hits_per_b.join(hits_per_bh, "app").selectExpr("app", "host", "bh_hits/total_b_hits as cond_p", "bh_hits/median_b_hits as host_importance")
...
cond_ps = compute_cond_p(ls_filtered, hits_per_b)
